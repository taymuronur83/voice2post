...
// --- VİDEO İÇERİĞİ (REMOTION RENDER İÇİN) ---
...
const MainSocialSystem = () => {
    ...
    // Video URL'sini yenilemek için cache-buster
    ...
    const handleGenerate = async () => {
        ...
        try {
            // 1. Adım: Metinleri ve Video Senaryosunu Üret
            ...
            // 2. Adım: GitHub Actions Render İşlemini Başlat
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', background: '#050505', color: '#eee', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: '#000' }}>
                <div style={{ width: '320px', height: '568px', border: '8px solid #1a1a1a', borderRadius: '40px', overflow: 'hidden', position: 'relative', background: '#111' }}>
                    {status === 'success' ? (
                        <video 
                            ...
                            onError={(e) => {
                                // Video henüz GitHub'a yüklenmemişse kullanıcıyı bilgilendir
                    ...
                    {/* Video hazırlanırken veya boşken gösterilecek alan */}
                    {(status !== 'success' || status === 'processing') && (
                        <div style={{ color: '#444', textAlign: 'center', marginTop: '70%', padding: '20px' }}>
                            {status === 'processing' ? 'Video Oluşturuluyor... (GitHub sunucusu çalışıyor)' : 'Komut Bekleniyor...'}
                        </div>
                    )}
                </div>
                {status === 'success' && (
                    <p style={{ color: '#666', fontSize: '11px', marginTop: '10px' }}>* Video birkaç dakika içinde burada belirecektir.</p>
                )}
            </div>
        </div>
    );
