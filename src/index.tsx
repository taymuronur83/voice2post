...
const SocialVideoContent = ({ title, sub, accentColor, storyline = [], animConfig }: { title: string; sub: string; accentColor: string; storyline?: string[]; animConfig?: any; }) => {
                ...
                <div style={{ width: '320px', height: '568px', border: '8px solid #1a1a1a', borderRadius: '40px', overflow: 'hidden', position: 'relative', background: '#111' }}>
                    {status === 'success' ? (
                        <video 
                            ...
                            controls autoPlay style={{ width: '100%', height: '100%', objectFit: 'cover' }} 
                        />
                    ) : (
                        <div style={{ color: '#444', textAlign: 'center', marginTop: '70%' }}>{status === 'processing' ? 'Video Hazırlanıyor...' : 'Komut Bekleniyor...'}</div>
                    )}
                </div>
            </div>
        </div>
